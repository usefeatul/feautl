    ...
    <div className="md:hidden fixed inset-0 z-[60] bg-background" data-component="MobileMenu">
      ...
      <div className="flex items-center justify-between h-14  px-4 sm:px-12 lg:px-16 xl:px-18">
        ...
        <button
          ...
          className="inline-flex items-center justify-center rounded-sm p-2 bg-muted/70"
          onClick={onClose}
        >
        ...
        <nav className="py-4 grid gap-2">
          {navigationConfig.main.map((item) => (
            <Link
              key={item.name}
              href={item.href}
              className="block rounded-sm px-2 py-2 text-lg text-accent hover:text-foreground hover:bg-muted"
              onClick={onClose}
            >
              {item.name}
            </Link>
          ...
          <div className="mt-4 grid gap-2">
            {navigationConfig.auth.map((item) => (
              <Link
                ...
                className="block rounded-sm px-3 py-2.5 mb-4 text-lg font-medium text-accent hover:text-foreground hover:bg-muted min-h-[36px]"
                onClick={onClose}
              >
                {item.name}
              </Link>
