...
export default function ChangelogSection({ slug }: { slug: string }) {
  const queryClient = useQueryClient()
  const { data = { isVisible: true, moderators: [] }, isLoading, refetch } = useQuery({
  ...
  React.useEffect(() => {
    setVisible(Boolean(data.isVisible))
  }, [data.isVisible])
