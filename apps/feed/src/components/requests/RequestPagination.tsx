...
export default function RequestPagination({ workspaceSlug, page, pageSize, totalCount, variant = "requests" }: Props) {
  ...
  const { totalPages, prevHref, nextHref, firstHref, lastHref } = useMemo(() => {
    ...
    return {
      ...
      firstHref: mk(workspaceSlug, params as URLSearchParams, { page: 1 }),
      lastHref: mk(workspaceSlug, params as URLSearchParams, { page: tp }),
    }
  ...
  return (
    <div className="mt-2 mb-2 flex w-full flex-col items-stretch justify-center gap-2 sm:mb-2 sm:flex-row sm:items-center sm:justify-between sm:gap-3">
      ...
      <div className="order-1 flex min-w-0 w-full flex-wrap items-center justify-center gap-2 sm:order-2 sm:w-auto sm:justify-start">
        <Button asChild variant="nav" size="xs" disabled={page <= 1}>
        ...
        <Button asChild variant="nav" size="xs" disabled={page >= totalPages || totalCount === 0}>
      ...
      <div className="order-3 flex w-full items-center justify-center gap-2 sm:w-auto sm:justify-end">
        ...
        <div className="-mx-6 flex items-center gap-1 overflow-x-auto whitespace-nowrap  sm:m-0 sm:overflow-visible">
          {sizes.map((s) => {
            const href = mk(workspaceSlug, params as URLSearchParams, { pageSize: s, page: 1 })
